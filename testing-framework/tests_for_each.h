...
#define TESTS_FOR_EACH_4(op, head, ...)                                                            \
    IF(TESTS_FOR_EACH_IS_END(head), CONSUME, TESTS_FOR_EACH_4_PROGRESS)(op, head, __VA_ARGS__)
#define TESTS_FOR_EACH_5(op, head, ...)                                                            \
    IF(TESTS_FOR_EACH_IS_END(head), CONSUME, TESTS_FOR_EACH_5_PROGRESS)(op, head, __VA_ARGS__)
#define TESTS_FOR_EACH_6(op, head, ...)                                                            \
    IF(TESTS_FOR_EACH_IS_END(head), CONSUME, TESTS_FOR_EACH_6_PROGRESS)(op, head, __VA_ARGS__)
#define TESTS_FOR_EACH_7(op, head, ...)                                                            \
    IF(TESTS_FOR_EACH_IS_END(head), CONSUME, TESTS_FOR_EACH_7_PROGRESS)(op, head, __VA_ARGS__)
#define TESTS_FOR_EACH_8(op, head, ...)                                                            \
    IF(TESTS_FOR_EACH_IS_END(head), CONSUME, TESTS_FOR_EACH_8_PROGRESS)(op, head, __VA_ARGS__)
#define TESTS_FOR_EACH_9(op, head, ...)                                                            \
    IF(TESTS_FOR_EACH_IS_END(head), CONSUME, TESTS_FOR_EACH_9_PROGRESS)(op, head, __VA_ARGS__)
